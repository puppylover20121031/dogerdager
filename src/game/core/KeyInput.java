...
public class KeyInput extends KeyAdapter {
  ...
  public KeyInput(Handler handler1, HUD hud, ArrowCode arrowcode) { 
    this.c = 0;
    this.r = new Random();
  ...
  public void keyPressed(KeyEvent keyEvent) { // conditions for key input
    ...
    for (int i = 0; i < Handler.object.size(); i++) {
      ...
      if (tempObject.getID() == ID.Player) {
        ...
        if (key == 12 && debug) {
          this.handler2
              .addObject(new SmartEnemy(this.r.nextInt(640), this.r.nextInt(427), ID.smartenemy, this.handler2));
        }
        if (key == 96 && debug) {
          hud1.setLevel(hud1.getLevel() + 1);
        }
      }
    }
  }
